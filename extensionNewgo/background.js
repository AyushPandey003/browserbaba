...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  ...
  if (request.action === 'fetchMemoriesFromAPI') {
    getMemoriesFromAPI(request.params || {})
      .then(result => {
        // API returns { success: true, data: memories[], count: number }
        // Extract the data array
        const apiMemories = (result.success && result.data) ? result.data : (Array.isArray(result) ? result : []);
        
        // Transform API response to match extension format
        const transformedMemories = apiMemories.map(memory => ({
          id: memory.id,
          user_id: memory.userId,
          url: memory.url,
          page_title: memory.title,
          title: memory.title,
          selected_text: memory.content || memory.selectedText || '',
          content: memory.content || memory.selectedText || '',
          content_type: mapMemoryTypeToContentType(memory.type),
          favicon: memory.metadata?.thumbnail || null,
          tags: memory.metadata?.tags || [],
          video_info: memory.metadata?.videoTimestamp ? {
            currentTime: memory.metadata.videoTimestamp,
            duration: memory.metadata.videoDuration,
            url: memory.metadata.videoUrl || memory.url,
            title: memory.metadata.videoTitle || memory.title
          } : null,
          metadata: memory.metadata || {},
          created_at: memory.createdAt || new Date().toISOString(),
          updated_at: new Date().toISOString()
        }));
        sendResponse({ success: true, data: transformedMemories });
      })
      .catch(error => {
        console.error('Error fetching from API:', error);
        sendResponse({ success: false, error: error.message });
...
// Map API memory types back to extension content types
function mapMemoryTypeToContentType(memoryType) {
  const typeMap = {
    'article': 'page',
    'note': 'text',
    'video': 'video',
  ...
  return typeMap[memoryType] || 'page';
}
