...
async function saveCurrentPage() {
  try {
    ...
    // Get favicon - prefer scraped metadata or tab icon
    let favicon = metadata.mainImage || null;
    if (!favicon && tab.favIconUrl && !tab.favIconUrl.startsWith('chrome://')) {
    ...
    // Prepare memory data with all enhanced fields
    const memoryData = {
      id: Date.now(),
      user_id: currentUser.id,
      url: tab.url,
      page_title: metadata.title || tab.title || 'Untitled',
      title: metadata.title || tab.title || 'Untitled',
      content_type: detectedType,
      selected_text: pageContent.substring(0, 1000),
      content: fullContent || pageContent,
      
      // Basic metadata
      domain: metadata.domain || new URL(tab.url).hostname,
      favicon: favicon,
